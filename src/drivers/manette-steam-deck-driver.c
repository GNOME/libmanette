              ...
              gint64                  time)
{
  ...
  if (self->last_left_stick_x != state->left_stick_x)
    send_absolute_event (self, MANETTE_AXIS_LEFT_X, state->left_stick_x, time, FALSE);

  if (self->last_left_stick_y != state->left_stick_y)
    send_absolute_event (self, MANETTE_AXIS_LEFT_Y, state->left_stick_y, time, TRUE);

  if (self->last_right_stick_x != state->right_stick_x)
    send_absolute_event (self, MANETTE_AXIS_RIGHT_X, state->right_stick_x, time, FALSE);

  if (self->last_right_stick_y != state->right_stick_y)
    send_absolute_event (self, MANETTE_AXIS_RIGHT_Y, state->right_stick_y, time, TRUE);
